...
export function pageTitle(html) {
  ...
  return decode(t).replace(/<[^>]+>/g, '').replace(/\s*[|｜].*$/, '').replace(/\s+/g, ' ').trim().normalize('NFKC').slice(0, 160);
}
...
const GENRE_RULES = [
  ...
  ['ガジェット・家電', /家電|イヤホン|スマホ|充電器|ドライヤー|掃除機|カメラ|ガジェット|スピーカー|美顔器/],
...
export function evaluate({source, url, html, now = new Date(), policy}) {
  ...
  const raw = {
    ...
    source, genre: classifyGenre(title, source), name: title, prize: title.slice(0, 120),
